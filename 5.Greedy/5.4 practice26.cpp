...
int main(){
    ...
    edge_map[1] = {{2,2}, {5,3}};       // 1-> 2 w: 2, 1->5 w:3
    edge_map[2] = {{1,2}, {5,5},{4,1}};
    edge_map[3] = {{4,2}, {7,3}};
    edge_map[4] = {{2,1}, {3,2}, {5,2}, {6,4}, {8,5}};
    edge_map[5] = {{1,2}, {2,5}, {4,2},{8,3},};
...
/*
    디스조인트-셋 자료구조를 사용하지 않는 크루스칼 알고리즘의 시간 복잡도는 O(E log E)
    E -> 에지의 개수
    그러나 사용한다면, O(Ea(V))로 줄어듬
    a(V) = 아커만 함수 : 전역적 재귀 함수의 예
    정점 개수에 따라 성능 차이가 있음(정점개수가 적으면 차이가 적음)
